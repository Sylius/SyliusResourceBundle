...
namespace Sylius\Resource\Symfony\Console\State;
...
final class ConsoleGridProvider implements ProviderInterface
{
    ...
    public function provide(Operation $operation, Context $context): object|array|null
    {
        ...
        $input = $context->get(ConsoleOption::class)?->input();
        ...
        if ($data instanceof PagerfantaInterface) {
            $currentPage = $input->hasOption('page') ? $input->getOption('page') : 1;
            $data->setCurrentPage($currentPage);

            $maxPerPage = $this->resolveMaxPerPage(
                $input->hasOption('limit') ? $input->getOption('limit') : null,
